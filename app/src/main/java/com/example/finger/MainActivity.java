...
//                	}	
                    ...
                    // Draw image
                    m_nImgWidth = w_nWidth[0]; m_nImgHeight = w_nHeight[0];
                    m_FpImageViewer.post(runShowStatus);	                
...
//                
    ...
    public void OnDetectFingerBtn(){
        ...
        if (!m_usbComm.IsInit())
            return;
        m_usbComm.oem_cmos_led(true);
        if (m_usbComm.oem_is_press_finger() < 0 ){
            m_txtStatus.setText("Communication error!");
            return;
        }
        if (m_usbComm.m_wLastAck == DevComm.NACK_INFO ){
            m_txtStatus.setText(DisplayErr( m_usbComm.m_nLastAckParam, 0 ));
            return;
        }
        if( m_usbComm.m_nLastAckParam != 0 ){
