...
import android.hardware.Finger;
...
public class DevComm {
	...
	public boolean IsInit() {
		return Finger.IsInit() == 1;
	}

	public boolean OpenComm() {
		return Finger.open() > 0;
	}
	...
	int Run_TestConnection() {
		...
		InitCmdPacket(CMD_TEST_CONNECTION, m_bySrcDeviceID, m_byDstDeviceID,
				m_abyPacket2, (short) 0);
	...
	int Run_SetParam(int p_nParamIndex, int p_nParamValue) {
		...
		InitCmdPacket(CMD_SET_PARAM, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, (short) 5);
	...
	int Run_GetParam(int p_nParamIndex, int[] p_pnParamValue) {
		...
		InitCmdPacket(CMD_GET_PARAM, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, (short) 1);
		...
		p_pnParamValue[0] = (int) ((int) ((m_abyPacket[RCM_DATA_OFFSET + 3] << 24) & 0xFF000000)
				...
				| (int) ((m_abyPacket[RCM_DATA_OFFSET + 1] << 8) & 0x0000FF00) | (int) (m_abyPacket[RCM_DATA_OFFSET + 0] & 0x000000FF));
	...
	int Run_GetDeviceInfo(String[] p_szDevInfo) {
		...
		InitCmdPacket(CMD_GET_DEVICE_INFO, m_bySrcDeviceID, m_byDstDeviceID,
				m_abyPacket2, 0);

		w_bRet = USB_SendPacket(CMD_GET_DEVICE_INFO);
		Log.e("com.example.noemhost", "USB_SendPacket w_bRet=" + w_bRet);
		if (!w_bRet)
			return ERR_CONNECTION;
		...
		w_nDevInfoLen = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 0],
				m_abyPacket[RCM_DATA_OFFSET + 1]);
		...
		System.arraycopy(m_abyPacket, RCM_DATA_OFFSET, m_abyPacket2, 0,
				w_nDevInfoLen);
	...
	int Run_SetIDNote(int p_nTmplNo, String p_pstrNote) {
		...
		InitCmdPacket(CMD_SET_ID_NOTE, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData2, 2);
	...
	int Run_GetIDNote(int p_nTmplNo, String[] p_pstrNote) {
		...
		w_abyData[0] = LOBYTE((short) p_nTmplNo);
		w_abyData[1] = HIBYTE((short) p_nTmplNo);
		InitCmdPacket(CMD_GET_ID_NOTE, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 2);
	...
	int Run_SetModuleSN(String p_pstrModuleSN) {
		...
		InitCmdPacket(CMD_SET_MODULE_SN, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData2, 2);
	...
	int Run_GetModuleSN(String[] p_pstrModuleSN) {
		...
		// . Assemble command packet
		InitCmdPacket(CMD_GET_MODULE_SN, m_bySrcDeviceID, m_byDstDeviceID,
				m_abyPacket2, 0);
	...
	int Run_GetImage() {
		...
		InitCmdPacket(CMD_GET_IMAGE, m_bySrcDeviceID, m_byDstDeviceID,
				m_abyPacket2, 0);
	...
	int Run_FingerDetect(int[] p_pnDetectResult) {
		...
		InitCmdPacket(CMD_FINGER_DETECT, m_bySrcDeviceID, m_byDstDeviceID,
				m_abyPacket2, 0);
	...
	/************************************************************************/
	/************************************************************************/
	int Run_UpImage(int p_nType, byte[] p_pFpData, int[] p_pnImgWidth,
			int[] p_pnImgHeight) {
		int w, h;
		boolean w_bRet;
		byte[] w_abyData = new byte[1];
		...
		InitCmdPacket(CMD_UP_IMAGE, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 1);
		...
		w = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 0],
				m_abyPacket[RCM_DATA_OFFSET + 1]);
		h = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 2],
				m_abyPacket[RCM_DATA_OFFSET + 3]);
		Log.e("com.example.noemhost", "w=" + w + "h=" + h);
	...
	int Run_SLEDControl(int p_nState) {
		...
		InitCmdPacket(CMD_SLED_CTRL, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 2);
	...
	int Run_StoreChar(int p_nTmplNo, int p_nRamBufferID, int[] p_pnDupTmplNo) {
		...
		InitCmdPacket(CMD_STORE_CHAR, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 4);
		...
		if (GetRetCode() != ERR_SUCCESS) {
			if (GetRetCode() == ERR_DUPLICATION_ID)
				p_pnDupTmplNo[0] = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 0],
						m_abyPacket[RCM_DATA_OFFSET + 1]);
	...
	int Run_LoadChar(int p_nTmplNo, int p_nRamBufferID) {
		...
		InitCmdPacket(CMD_LOAD_CHAR, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 4);
	...
	int Run_UpChar(int p_nRamBufferID, byte[] p_pbyTemplate) {
		...
		InitCmdPacket(CMD_UP_CHAR, m_bySrcDeviceID, m_byDstDeviceID, w_abyData,
				2);
		...
		System.arraycopy(m_abyPacket, RCM_DATA_OFFSET, p_pbyTemplate, 0,
				GD_RECORD_SIZE);
	...
	int Run_DownChar(int p_nRamBufferID, byte[] p_pbyTemplate) {
		...
		InitCmdPacket(CMD_DOWN_CHAR, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData2, 2);
	...
	int Run_DelChar(int p_nSTmplNo, int p_nETmplNo) {
		...
		// . Assemble command packet
		InitCmdPacket(CMD_DEL_CHAR, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 4);
	...
	int Run_GetEmptyID(int p_nSTmplNo, int p_nETmplNo, int[] p_pnEmptyID) {
		...
		// . Assemble command packet
		InitCmdPacket(CMD_GET_EMPTY_ID, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 4);
		...
		p_pnEmptyID[0] = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 0],
				m_abyPacket[RCM_DATA_OFFSET + 1]);
	...
	int Run_GetStatus(int p_nTmplNo, int[] p_pnStatus) {
		...
		InitCmdPacket(CMD_GET_STATUS, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 2);
	...
	/************************************************************************/
	/************************************************************************/
	int Run_GetBrokenID(int p_nSTmplNo, int p_nETmplNo, int[] p_pnCount,
			int[] p_pnFirstID) {
		boolean w_bRet;
		byte[] w_abyData = new byte[4];
		...
		// . Assemble command packet
		InitCmdPacket(CMD_GET_BROKEN_ID, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 4);
		...
		p_pnCount[0] = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 0],
				m_abyPacket[RCM_DATA_OFFSET + 1]);
		p_pnFirstID[0] = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 2],
				m_abyPacket[RCM_DATA_OFFSET + 3]);
	...
	int Run_GetEnrollCount(int p_nSTmplNo, int p_nETmplNo, int[] p_pnEnrollCount) {
		...
		// . Assemble command packet
		InitCmdPacket(CMD_GET_ENROLL_COUNT, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 4);
		...
		p_pnEnrollCount[0] = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 0],
				m_abyPacket[RCM_DATA_OFFSET + 1]);
	...
	int Run_Generate(int p_nRamBufferID) {
		...
		InitCmdPacket(CMD_GENERATE, m_bySrcDeviceID, m_byDstDeviceID,
				w_abyData, 2);
	...
	/************************************************************************/
	/************************************************************************/
	int Run_Search(int p_nRamBufferID, int p_nStartID, int p_nSearchCount,
			int[] p_pnTmplNo, int[] p_pnLearnResult) {
		boolean w_bRet;
		...
		InitCmdPacket(CMD_SEARCH, m_bySrcDeviceID, m_byDstDeviceID, w_abyData,
				6);
		...
		p_pnTmplNo[0] = MAKEWORD(m_abyPacket[RCM_DATA_OFFSET + 0],
				m_abyPacket[RCM_DATA_OFFSET + 1]);
	...
	int Run_Verify(int p_nTmplNo, int p_nRamBufferID, int[] p_pnLearnResult) {
		...
		// . Assemble command packet
		InitCmdPacket(CMD_VERIFY, m_bySrcDeviceID, m_byDstDeviceID, w_abyData,
				4);
	...
	public boolean GetDeviceInformation(String[] deviceInfo) {
		...
		Log.e("com.example.noemhost", "SendPackage=" + w_bRet);
		...
		Log.e("com.example.noemhost", "RecvPackage=" + w_bRet);
		// Toast.makeText(mApplicationContext,
	...
	private boolean SendPackage(byte[] pPCCmd, byte[] pData) {
		...
		return UsbSCSIWrite(pPCCmd, 6, pData, nDataLen, 5000);
	}
	...
	private boolean RecvPackage(byte[] pData, int[] pLevRen) {
		...
		// receive status
		w_bRet = UsbSCSIRead(w_abyPCCmd, 6, w_abyRespond, 4, 5000);
		...
		if (w_nLen > 0) {
			...
			w_abyPCCmd[1] = 0x03;
			w_bRet = UsbSCSIRead(w_abyPCCmd, 6, pData, w_nLen, 5000);
	 ...
	 * Make Command Packet
	 ***************************************************************************/
	void InitCmdPacket(short wCMDCode, byte bySrcDeviceID, byte byDstDeviceID,
			byte[] pbyData, int nDataLen) {
		short w_wCheckSum;
	 ...
	 * Make Data Packet
	 ***************************************************************************/
	void InitCmdDataPacket(short wCMDCode, byte bySrcDeviceID,
	...
	private boolean USB_SendPacket(short wCMD) {
		...
		UsbSCSIWrite(btCDB, 8, m_abyPacket, m_nPacketSize, SCSI_TIMEOUT);
	...
	private boolean USB_ReceiveAck(short wCMD) {
		...
		do {
			...
			if (!UsbSCSIRead(btCDB, 8, m_abyPacket, w_nLen, SCSI_TIMEOUT)) {
				return false;
			}
	...
	boolean USB_ReceiveDataAck(short wCMD) {
		...
		if (!CheckReceive(m_abyPacket, m_nPacketSize,
				(short) RCM_DATA_PREFIX_CODE, wCMD)) {
			return false;
		}
	...
	boolean USB_SendDataPacket(short wCMD) {
		...
		if (!UsbSCSIWrite(btCDB, 8, m_abyPacket, m_nPacketSize, SCSI_TIMEOUT))
			return false;
	...
	boolean USB_ReceiveRawData(byte[] pBuffer, int nDataLen) {
		...
		if (!UsbSCSIRead(btCDB, 8, pBuffer, nDataLen, SCSI_TIMEOUT))
			return false;

		return true;
	}

	boolean GetHead(byte[] buf, int nIndex, int nCount, int[] nResult) {
		int nStar = nIndex, nEnd = nIndex + nCount;
		while (nStar < nEnd) {
			if ((buf[nStar] & 0xff) == 0xa5) {
				nStar++;
				if ((buf[nStar] & 0xff) == 0x5a) {
					nResult[0] = ++nStar;
					return true;
				}
			}
	...
	int ReadOnePacket(byte[] pin, int nIndex, int nCount, byte[] pout,
			int nOutIndex, int[] nResult) {
		int[] nStar = new int[1];
		nResult[0] = 0;
		if (GetHead(pin, nIndex, nCount, nStar)) {
			int len = 0xff & pin[9 + nStar[0]];
			len <<= 8;
			len |= (pin[8 + nStar[0]] & 0xff);
			if ((pout.length > (nOutIndex + len))
					&& (pin.length > (nStar[0] + 10 + len))) {
				// Log.e("123",
				// "poutlen="+pout.length+"nOutIndex="+nOutIndex+"len="+len+"gdgd="+(nStar[0]+10));
				System.arraycopy(pin, nStar[0] + 10, pout, nOutIndex, len);
			}

			nResult[0] = nStar[0] + 12 + len;
			return len;
		}
		return 0;
	}
	...
	boolean USB_ReceiveImage(byte[] p_pBuffer, int nDataLen) {
		int nCount = 0, nlen = 0, ntem = 0, nLentemp = 0, ntime = 0;
		byte[] buf = new byte[20];
		nDataLen = (nDataLen / 496) * (496 + 14) + (nDataLen % 496 + 14);
		byte[] buftemp = new byte[nDataLen + 20];
		while ((nCount < nDataLen) && (ntime < 2)) {
			// Log.e("123",
			// "nCount="+nCount+"ntem="+ntem+"nDataLen="+nDataLen+"ntime="+ntime);
			// Log.e("123",
			// "nCount="+nCount+"nDataLen-nCount="+(nDataLen-nCount));
			nLentemp = Finger.readbuf(buftemp, nCount, nDataLen - nCount,
					5000000);
			if ((nCount > 0) && (nLentemp == 0))
				ntime++;
			else if (nLentemp > 0) {
				ntime = 0;
			}
		...
		Log.e("123", "nCount=" + nCount + "nDataLen=" + nDataLen);
		// nlen=0;
		// for(int i=0;i<nCount/16;i++)
		// {
		// Log.e("123", ""+Finger.BytesToString(buftemp, i*16, 16));
		// }
		// int left=nCount%16;
		// if(left>0)
		// {
		// Log.e("123", ""+Finger.BytesToString(buftemp,nCount-left, left));
		// }
		// if(nCount>=nDataLen)
		{
			// Log.e("123", "copy") ;
			...
			ntem = 0;
			ntime = 0;
			while ((nlen < nDataLen) && (ntime < 500)) {
				ntime++;
				nread = ReadOnePacket(buftemp, nlen, nDataLen - nlen,
						p_pBuffer, ntem, nStar);
				if ((nread == 0) || (ntem > 64000))
					break;
				ntem += nread;
				// Log.e("123","nread="+nread+"ntem="+ntem+"nStar="+nStar[0]);
				nlen = nStar[0];
			}
		}
		// else
	...
	private boolean UsbSCSIWrite(byte[] pCDB, int nCDBLen, byte[] pData,
			int nDataLen, int nTimeOut) {
		Finger.Clear();
		Finger.writedata(pData, 0, nDataLen);
		return true;

	}

	public boolean UsbSCSIRead(byte[] pCDB, int nCDBLen, byte[] pData,
			int nDataLen, int nTimeOut) {
		// long w_nTime;
		// w_nTime = SystemClock.elapsedRealtime();
		// Log.e("com.example.noemhost","nTimeOut="+nTimeOut);
		int offset = 0, ncount = 0, nlen = 0;
		nTimeOut /= 100;
		while (ncount < nTimeOut) {
			nlen = Finger.readdata(pData, offset, nDataLen - offset);

			offset += nlen;
			// Log.e("com.example.noemhost", "nlen=" + nlen + "offset=" +
			// offset);
			if (nlen == 0) {
				ncount++;
				try {
			...
			if ((offset >= nDataLen) || ((offset > 0) && (ncount > 5)))
				break;
		}

		// Log.e("com.example.noemhost", "end offset=" + offset);
		return offset > 0;
	}

	public int UsbSCSIReadbuf(byte[] pData, int nIndex, int nDataLen,
			int nTimeOut) {
	...
	public boolean UsbSCSIReadEx(byte[] pData, int nIndex, int nDataLen,
			int nTimeOut) {
		// long w_nTime;
		// w_nTime = SystemClock.elapsedRealtime();
		// Log.e("com.example.noemhost","nTimeOut="+nTimeOut);
		...
		nTimeOut /= 100;
		while (ncount < nTimeOut) {
			Log.e("com.example.noemhost", "nlen=" + nlen + "offset=" + offset);
			nlen = Finger.readdata(pData, nIndex + offset, nDataLen - offset);
